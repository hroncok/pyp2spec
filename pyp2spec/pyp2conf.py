...
class PypiPackage:
    ...
    def get_license_from_classifiers(self, compliant):
        ...
        classifiers = self.read_license_classifiers()

        if classifiers is None:
            # No classifiers were defined, no need to check further
            return False
        ...
        for classifier in classifiers:
            ...
            if compliant:
                if fedora_status == "BAD":
                    click.secho(f"License '{short_license}' is not allowed in Fedora, quitting", fg='red')
                    exit(1)
                ...
                elif fedora_status in ["UNKNOWN", "???"]:
                    click.secho(f"License '{short_license}' may not be allowed in Fedora, quitting", fg='red')
                    exit(1)
                else:
